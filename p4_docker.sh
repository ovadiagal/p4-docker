...
release="latest"
imageBaseName="galovadia03/p4-tools-image"
...
# --priviledged is needed for Apple Silicon MacOS compatibility
if [ "$action" = "start" ]; then
  docker run --privileged --rm -v "$currDir":/usr/src/app/local/ --cap-add=SYS_PTRACE -w /usr/src/app/local --security-opt seccomp=unconfined -it "$imageName"
fi
